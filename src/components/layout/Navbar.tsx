		...
		<header className="fixed top-0 z-50 w-full border-b border-border bg-white/90 backdrop-blur-sm">
			<div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 md:px-10 lg:px-16">
				...
				<div className="hidden items-center gap-2 md:flex">
					...
					<Button variant="solid" size="sm" trailingIcon={<ArrowRight size={14} aria-hidden="true" />}>
					...
					<Menu.Root>
						...
						<Menu.Portal>
							<Menu.Positioner sideOffset={8} align="end">
								<Menu.Popup className="min-w-48 origin-[var(--transform-origin)] rounded-2xl border border-border bg-background py-1 shadow-lg outline-hidden transition-[scale,opacity] duration-100 data-ending-style:scale-[0.98] data-ending-style:opacity-0 data-starting-style:scale-[0.98] data-starting-style:opacity-0">
									{navLinks.map((link) => (
										<Menu.Item key={link.label} className={menuItemClass} render={<a href={link.href} />}>
											{link.label}
										</Menu.Item>
									))}
									<Menu.Separator className="mx-2 my-1 h-px bg-border" />
									<Menu.Item className={cn(menuItemClass, 'font-medium')}>Become a Provider</Menu.Item>
									<Menu.Item className={cn(menuItemClass, 'font-medium')}>Request Biospecimens</Menu.Item>
								</Menu.Popup>
							</Menu.Positioner>
						</Menu.Portal>
					</Menu.Root>
